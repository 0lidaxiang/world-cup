...
| 优先顺序 | ① 项目内已有 CSV/文档 ② 单次批量官方公开数据 ③ 必要时才联网抓取 |
| 失败重试 | 遇 429/503 时 **指数退避**（建议 2s → 4s → 8s），不得立即连发重试 |

当前批次生成脚本（`scripts/generate_*.py`）以**本地结构化写入**为主，不访问外网；**今后凡新增联网抓取逻辑，必须使用** [`scripts/fetch_utils.py`](../scripts/fetch_utils.py)。

---

## 二、Agent 执行 SOP 补充

1. 领 `task_type=data` 任务前，确认 `data_source_hint` 是否必须联网；能用手工/已有资料编写字段的，**不要**为省事开爬虫。
...
- `min_interval_sec` 不得小于 **1.0**（除非任务 notes 明确写明已获书面授权且调大仍不足，需人工审批）。
...
- 校验/合并阶段不检查网速；若发现脚本高频访问导致封禁，将任务标为 `blocked` 并在 `notes` 说明。
- Review 任务（`task_type=review`）不触发外网抓取。
