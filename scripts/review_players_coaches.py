#!/usr/bin/env python3
"""Review knowledge_players_coaches.csv quality (T190).
...
TARGET = ROOT / "data" / "knowledge_players_coaches.csv"
REPORT = ROOT / "docs" / "reviews" / "T190-players-coaches-review.md"
EXPECTED_ROWS = 1500
SAMPLE_SIZE = 50
ID_PREFIX = "WC-PLCO-"
...
def main() -> int:
    ...
    bad_l1 = [r["id"] for r in rows if r.get("category_l1") != "球员与教练"]
    if bad_l1:
    ...
    batch_sizes = Counter(len(v) for v in by_l2.values())
    if batch_sizes.get(50, 0) != len(by_l2) and len(by_l2) == 30:
    ...
    lines = [
        "# T190 球员与教练质量抽检报告",
        "",
        f"- 文件: `data/knowledge_players_coaches.csv`",
        ...
        f"| category_l1 球员与教练 | {'PASS' if not bad_l1 else 'FAIL'} |",
    ...
    if ok:
        lines.append("**通过**：全量校验零 error，1500 条球员与教练知识可标 T190 done。")
        code = 0
    else:
        lines.append("**未通过**")
        for i in issues[:40]:
