#!/usr/bin/env python3
"""Review knowledge_national_teams.csv quality (T150).
...
TARGET = ROOT / "data" / "knowledge_national_teams.csv"
REPORT = ROOT / "docs" / "reviews" / "T150-national-teams-review.md"
EXPECTED_ROWS = 1200
SAMPLE_SIZE = 60
...
def main() -> int:
    ...
    bad_prefix = [i for i in ids if not i.startswith("WC-NTEM-")]
    if bad_prefix:
    ...
    if nums and (nums[0] != 1 or nums[-1] != len(rows) or len(nums) != len(set(nums))):
    ...
    lines = [
        "# T150 国家队质量抽检报告",
        "",
        f"- 文件: `data/knowledge_national_teams.csv`",
        f"- 总行数: **{len(rows)}**（目标 {EXPECTED_ROWS}）",
        f"- category_l2 批次数: **{len(by_l2)}**",
        f"- 全量校验: **{'PASS' if validate.returncode == 0 else 'FAIL'}**",
        f"- 等距抽检: **{len(sample)}** 条（约 5%）",
        "",
        "| 检查项 | 结果 |",
        "|--------|------|",
        f"| validate_knowledge --strict | {'PASS' if validate.returncode == 0 else 'FAIL'} |",
        f"| ID 唯一 WC-NTEM- | {'PASS' if not bad_prefix and len(ids) == len(set(ids)) else 'FAIL'} |",
        f"| ID 连续 00001–{EXPECTED_ROWS:05d} | {'PASS' if len(rows) == EXPECTED_ROWS and not any('sequence' in i for i in issues) else 'FAIL'} |",
    ...
    if ok:
        lines.append("**通过**：全量校验零 error，1200 条国家队知识可标 T150 done。")
        code = 0
    else:
        lines.append("**未通过**")
        for i in issues[:40]:
